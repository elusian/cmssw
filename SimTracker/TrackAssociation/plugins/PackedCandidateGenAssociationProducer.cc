...
void PackedCandidateGenAssociationProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  //std::clog << recoGenCollection.id() << std::endl;
...
//  std::clog << "Assoc id: " << hgenToPrunedAssoc.id() << std::endl;
//  std::clog << "Assoc dest id: " << prunedCollection.id() << std::endl;
//  
//  std::clog << "Contained ids:" << std::endl;
//  for (auto&& [id, index]: genToPrunedAssoc.ids()) {
